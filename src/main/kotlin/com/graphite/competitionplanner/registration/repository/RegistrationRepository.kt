...
class RegistrationRepository(
    ...
    fun getRegistrationStatus(playerId: Int, competitionCategoryId: Int): PlayerRegistrationRecord? {
        return dslContext
                .select()
                ...
                .where(PLAYER_REGISTRATION.PLAYER_ID.eq(playerId)
                        .and(COMPETITION_CATEGORY_REGISTRATION.COMPETITION_CATEGORY_ID.eq(competitionCategoryId)))
                .fetchOneInto(PLAYER_REGISTRATION)
    }
    ...
    override fun getAllRegisteredPlayersInCompetitionCategory(
            competitionCategoryId: Int): List<Pair<Registration, PlayerWithClubDTO>> {
        val records = dslContext.select(
                PLAYER.ID,
                PLAYER.FIRST_NAME,
                PLAYER.LAST_NAME,
    ...
    override fun unregisterIndividualPlayer(registrationId: Int, playerId: Int) {
        ...
        if (!success) {
            throw NotFoundException(
                    "Could not delete. The registration with id $registrationId and playerId $playerId was not found.")
        }
    }
    ...
    override fun getRegistrationIdForPlayerInCategory(categoryId: Int, playerId: Int): Int {
        return dslContext
                ...
                ?: throw NotFoundException(
                        "That combination of category ID $categoryId and player ID $playerId was not found")
    }
